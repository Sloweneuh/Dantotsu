...
class ComickInfoFragment : Fragment() {
     ...
     * titles with CJK (Chinese/Japanese/Korean) characters
     */
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (offline) {
            loaded = true
            showError(getString(R.string.comick_requires_internet))
            return
        }
            ...
            try {
                ...
                withContext(Dispatchers.Main) {
                    if (comickData == null) {
                        ...
                        Toast.makeText(
                                        requireContext(),
                                        getString(R.string.failed_fetch_comick),
                                        Toast.LENGTH_SHORT
                                )
                                .show()
                        return@withContext
                    ...
                    val frameLayout =
                            binding.root
                                    .findViewById<ViewGroup>(R.id.mediaInfoScroll)
                                    ?.getChildAt(0) as?
                                    ViewGroup
                    ...
                    Toast.makeText(
                                    requireContext(),
                                    getString(R.string.linked_comick_entry),
                                    Toast.LENGTH_SHORT
                            )
                            .show()
                }
    ...
    private fun unlinkComickSelection(media: Media) {
        ...
        Toast.makeText(
                        requireContext(),
                        getString(R.string.comick_entry_unlinked),
                        Toast.LENGTH_SHORT
                )
                .show()
    }
    ...
    private fun showNoDataWithSearch(media: Media) {
        ...
        frameLayout?.let { container ->
            ...
            // Set logo
            pageView.findViewById<android.widget.ImageView>(ani.dantotsu.R.id.logo)
                    ?.setImageResource(ani.dantotsu.R.drawable.ic_round_comick_24)
    ...
    private fun displayComickInfo(comickData: ComickResponse) {
        ...
        val romajiTitle =
                romajiLang?.let { lang -> comic.md_titles?.firstOrNull { it.lang == lang }?.title }
        ...
        binding.mediaInfoPopularity.parent?.let { tableRow ->
            if (tableRow is ViewGroup && tableRow.childCount > 0) {
                val label = tableRow.getChildAt(0)
                if (label is android.widget.TextView) {
                    label.text = getString(R.string.followers)
                }
            }
        }
        // Followers (user_follow_count)
        binding.mediaInfoPopularity.text =
                comic.user_follow_count?.toString() ?: getString(R.string.unknown_value)
        ...
        binding.mediaInfoFavorites.parent?.let { tableRow ->
            if (tableRow is ViewGroup && tableRow.childCount > 0) {
                val label = tableRow.getChildAt(0)
                if (label is android.widget.TextView) {
                    label.text = getString(R.string.ranking)
                }
            }
        }
        // Ranking (follow_rank)
        binding.mediaInfoFavorites.text =
                "#${comic.follow_rank ?: getString(R.string.unknown_value)}"
        ...
        if (!mdTitles.isNullOrEmpty()) {
            ...
            if (englishTitles.isNotEmpty() &&
                ...
                englishTitles.forEach { title ->
                    val chip =
                            ItemChipSynonymBinding.inflate(
                                            LayoutInflater.from(context),
                                            bind.itemChipGroup,
                                            false
                                    )
                    ...
                    chip.setOnLongClickListener {
                        ...
                        Toast.makeText(
                                        requireContext(),
                                        getString(R.string.copied_title_toast, title),
                                        Toast.LENGTH_SHORT
                                )
                                .show()
                        true
            ...
            if (parent.findViewWithTag<View>("anime_info_comick") == null) {
                ...
                val infoText = buildString {
                    if (!animeInfo.start.isNullOrBlank()) {
                        append(getString(R.string.anime_start_format, animeInfo.start))
                    }
                    if (!animeInfo.end.isNullOrBlank()) {
                        append(getString(R.string.anime_end_format, animeInfo.end))
                    }
                }
        ...
        if (!categories.isNullOrEmpty()) {
            ...
            if (parent.findViewWithTag<View>("tags_comick") == null) {
                ...
                sortedCategories.forEach { category ->
                    ...
                    chip.setOnLongClickListener {
                        copyToClipboard(title)
                        Toast.makeText(requireContext(), "Copied: $title", Toast.LENGTH_SHORT)
                                .show()
                        true
                    }
        ...
        if (!recommendations.isNullOrEmpty() &&
                        parent.findViewWithTag<View>("recommendations_comick") == null
        ) {
            viewLifecycleOwner.lifecycleScope.launch {
                val model: MediaDetailsViewModel by activityViewModels()
