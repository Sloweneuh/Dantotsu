...
/**
 * The body of [updateProgress] as a suspending call: writes [number] to whichever tracker backs
 * [media] (MangaUpdates for a MU-only entry, otherwise AniList) and suspends until that primary
 * write is done, then fires the mirror writes (MAL, MangaBaka, cloud sync) without waiting.
 *
...
suspend fun updateProgressSuspending(media: Media, number: String) {
    ...
    // only for the primary tracker write, not for every mirror round trip behind it.
    val mirrorScope = CoroutineScope(Dispatchers.IO)
